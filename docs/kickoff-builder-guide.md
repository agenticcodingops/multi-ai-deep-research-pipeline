...
| `scripts/test_*.py`, `scripts/fixtures/kickoff/` | 216 tests; every pass fixture is generated by the golden loop (fixtures are asserted byte-equal to production-code regeneration, then validated PASS) |
...
Verification, from the repo root (use `python3` instead of `python` on POSIX surfaces where `python` is absent):
